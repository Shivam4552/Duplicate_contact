...
def fetch_todays_contacts():
    """Fetch all contacts created today"""
    url = "https://api.hubapi.com/crm/v3/objects/contacts/search"
    all_contacts = []
    after = None
    fetched = 0

    print(f"🔍 Fetching all contacts created TODAY ({TODAY.strftime('%Y-%m-%d')})...")
...
def process_all_todays_duplicates():
    """Main function to process all today's duplicate contacts"""
    
    print("🚀 PROCESSING ALL TODAY'S DUPLICATE CONTACTS")
    print(f"📅 Date: {TODAY.strftime('%Y-%m-%d')}")
    print("🎯 Using HubSpot-compliant pairwise merge strategy")
    print("=" * 80)
    
    # Fetch today's contacts
    ...
    print("=" * 60)
    print(f"📅 Date Processed: {TODAY.strftime('%Y-%m-%d')}")
    print(f"📧 Total Contacts Today: {len(contacts)}")
    ...
    print(f"\n🎉 Processing Complete for {TODAY.strftime('%Y-%m-%d')}!")

# ========== Main Logic ==========

def main():
    print("🚀 TODAY'S DUPLICATE CONTACT PROCESSOR")
    print("📅 Processes ALL contacts created today")
    print("🎯 HubSpot-compliant pairwise merge strategy")
    print("⚡ Handles 2-contact merges automatically, 3-contact strategically")
    print("=" * 80)
    
    try:
        process_all_todays_duplicates()
    except Exception as e:
