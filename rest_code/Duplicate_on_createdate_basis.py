...
def fetch_contacts_by_date(start_date, end_date, limit=15000):
    """Fetch contacts created between start_date and end_date"""
...
def find_duplicates_for_specific_date(target_date):
    """Find all duplicate contacts created on the specified date (from 12:00 AM to 11:59:59 PM)"""
    ...
    print("\n" + "="*60)
    print(f"📧 EMAIL DUPLICATES FOUND ON {target_date.strftime('%Y-%m-%d')} (00:00 - 23:59 IST):")
    print("="*60)
    ...
    print("\n" + "="*60)
    print(f"📱 PHONE DUPLICATES FOUND ON {target_date.strftime('%Y-%m-%d')} (00:00 - 23:59 IST):")
    print("="*60)
...
def find_duplicates_for_date_range(start_date, days_range=7):
    """Find duplicates for multiple dates (useful for checking past week)"""
    print(f"\n🔍 Searching for duplicates over {days_range} days starting from {start_date.strftime('%Y-%m-%d')}...")
    ...
    for days_back in range(days_range):
        ...
        print(f"\n📅 Checking {date_str}...")
        email_dups, phone_dups = find_duplicates_for_specific_date(current_date)
...
def main():
    print("🚀 Starting Duplicate Contact Detection...")
    ...
    range_results = find_duplicates_for_date_range(TARGET_DATE, days_range=7)
    if range_results:
